...
from neural_extractor.validator import extract_video_id
...
class SubtitleDownloader:
    ...
    def download_subtitles(
        ...
        results: dict[str, Optional[Path]] = {fmt: None for fmt in formats}
    ...
    def _download_via_transcript_api(
        ...
        try:
            ...
            try:
                ...
                try:
                    manual_transcripts = [
                        t for t in transcript_list if not t.is_generated  # type: ignore
                    ]
            ...
            if not transcript:
                try:
                    generated_transcripts = [
                        t for t in transcript_list if t.is_generated  # type: ignore
                    ]
    ...
    def _convert_srt_to_vtt(
        self, srt_path: Path, video_id: str, video_title: Optional[str] = None
    ) -> Optional[Path]:
        """
    ...
    def _convert_vtt_to_srt(
        self, vtt_path: Path, video_id: str, video_title: Optional[str] = None
    ) -> Optional[Path]:
        """
