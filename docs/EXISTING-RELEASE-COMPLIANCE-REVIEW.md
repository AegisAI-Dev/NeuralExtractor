...
| Provider-free one-file 3.0.8 (`45b423…`) | **Confirmed absent**; PySide6/Qt present | **Confirmed absent** from main EXE; optional helper separate | Fully inventoried in the artifact companion | Improved embedded notices/manifests; project ownership metadata is now resolved, but LGPL replacement, FFmpeg/Python source closure, Microsoft rights, Qt/native closure and legal approval remain unresolved | Keep non-public on HOLD. Use only as audit evidence while building the preferred compliance-friendly one-folder/source package. |
